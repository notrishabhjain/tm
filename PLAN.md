...
_Justification: `expo-sqlite` v14 = JSI-based, New Arch native. Drizzle is the only type-safe SQLite ORM with Expo integration. MMKV is the only performant synchronous KV store compatible with New Arch._
...
_Justification: Notifee is the only library with advanced notification channels, triggers, and quick actions under New Arch. `expo-background-fetch` for WorkManager-backed scheduled jobs._
...
_Justification: Reanimated 3 = worklet-only animations (required). GestureHandler = swipe gestures. FlashList = required (no FlatList). BottomSheet = filter/options sheets. SVG for illustrations. Lucide = consistent icon library._
...
_Justification: react-hook-form for all forms. Zustand for UI state. TanStack Query for reactive DB subscriptions pattern._
...
_Justification: device-info for OEM detection (F-20). expo-calendar for F-10. file-system + sharing for F-12 export/import._
...
_Justification: i18next is the standard. No cloud dependency. Fully offline._
...
_Justification: Only maintained ONNX runtime for React Native. New Arch compatible._
...
_Justification: Catches uncaught JS errors, writes crash log to storage for Diagnostics export. No network calls._
...
export function logCapturedNotification(
  ...
  notificationBuffer = [{ ...data, status, capturedAt: Date.now() }, ...notificationBuffer].slice(
    0,
    BUFFER_SIZE
  );
